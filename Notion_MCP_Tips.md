...
### 追記（2026-08-18）: 複数行をまとめてold_strにすると一致しないことがある

`content_updates`で複数行（例: 4行）をまとめて1つの`old_str`に指定すると、視覚的には内容が一致しているにもかかわらず「一致なし」エラーになることがある（Unicode正規化・IME変換由来の文字表現の違いが原因と推測される）。**`old_str`は複数行・複数箇所をまとめず、1箇所・1〜2行程度の短い範囲に絞って送る**のが最も再現性高く回避できる。複数件の更新がある場合も1件ずつ個別に送信する。
...
## 18. content_updates の new_str でバックスラッシュを含む文字列をコードスパン外に書くと消える

### 症状

`update_content`の`new_str`に`no-git-backup\claude-memory`のようにバックスラッシュを含む文字列をバッククォートで囲まずに書いて送信すると、ツール呼び出しはエラーなく成功するが、実際にページへ反映された文字列からバックスラッシュだけが消える（`no-git-backupclaude-memory`のように結合されてしまう）。

### 原因

Notion Markdownのパーサーがバックスラッシュを標準的なエスケープシーケンスの開始と解釈し、直後の文字だけを残してバックスラッシュ自体を除去するため。

### 対処法

Windowsパスなど、バックスラッシュを含む文字列は必ずコードスパン（`` `...` ``）で囲んで送る。コードスパン内はMarkdownのインライン記法が適用されないため、バックスラッシュがそのまま保持される。

例: `no-git-backup\claude-memory`（誤、バッククォートなし） → `` `no-git-backup\claude-memory` ``（正）

### 教訓

反映後は`notion-fetch`で再取得し、意図した文字列（特にバックスラッシュ・アンダースコア等の記号）がそのまま残っているか確認する。

---

## 19. ファイル名（拡張子付き）がURLとして自動リンク化されることがある

### 症状

`Notion_MCP_Tips.md`のような「単語＋ドット＋拡張子」を含む文字列をバッククォートなしで本文に書くと、ツール呼び出しはエラーなく成功するが、Notion側の自動リンク化（オートリンク）機能が拡張子部分（例: `.md`）をドメインの一部と誤認識し、`Notion_MCP_[Tips.md](http://Tips.md)`のように文字列の一部だけが勝手にURLへのリンクへ変換されてしまう。

### 原因

Notionはリッチテキスト変換時に「単語.拡張子」のようなパターンをURLと推測して自動リンク化する。`.md`は実在の国別ドメイン（モルドバ）でもあるため、ファイル名の拡張子でも誤ってヒットする。

### 対処法

ファイル名・パスなど、ドット＋拡張子を含む文字列は必ずコードスパン（`` `...` ``）で囲んで送る。項目18（バックスラッシュの消失）と同じ対策で、コードスパン内はMarkdownのインライン記法（自動リンク化含む）が適用されないため防げる。

例: `Notion_MCP_Tips.md`（誤、バッククォートなし） → `` `Notion_MCP_Tips.md` ``（正）

### 教訓

反映後は`notion-fetch`で再取得し、ファイル名・パスが意図しないリンクに変換されていないか確認する。

---

## 20. Notion MCPには削除・アーカイブ用のツールが存在しない

### 症状

ページを削除（またはアーカイブ／ゴミ箱移動）したいという依頼に対し、対応するツールを探しても見つからない。`notion-move-pages`（親の変更）はあるが、削除系のツールは提供されていない。

### 原因

MCPサーバー側がそもそも削除・アーカイブ用のツールを公開していない（`notion-search`/`notion-fetch`/`notion-create-pages`/`notion-update-page`/`notion-move-pages`/`notion-update-data-source`/`notion-create-view`/`notion-update-view`等は提供されるが、削除系は含まれない）。

### 対処法

削除は**MCPツールセットの制約でありワークアラウンドの余地がない**。代替手段（別プロパティでの移動、ステータスを完了扱いにする等）を独自に模索する前に、まずこの制約自体を呼び出し元へそのままエラーとして返す。呼び出し元（ユーザー）に、削除の代わりにどう扱うか（放置／Web UIで手動削除／ステータス変更等）を判断してもらう。

### 教訓

ツールが存在しない場合は、無理に別ツールの組み合わせで代替しようと試行錯誤せず、「このツールセットではできない」という事実を早期に報告する方が確実で速い。

---

## 21. URL直後に全角括弧で説明文を続けると、括弧内までまとめて自動リンク化される

### 症状

`https://code.claude.com/docs/en/hooks.md（PreCompactイベントの仕様）`のように、URLの直後に区切り文字なしで全角括弧の日本語説明文を続けて書くと、ツール呼び出しはエラーなく成功するが、Notion側の自動リンク化がURL部分だけでなく括弧内の日本語説明文までまとめて1本のリンクとして解釈してしまい、リンク先が壊れる（本来のURLへ正しく飛ばない）。

### 原因

項目19と同様、Notionのオートリンク機能がURLの終端を正しく判定できず、直後に続く全角括弧のテキストまで同一リンクの一部とみなしてしまう。

### 対処法

URLと日本語説明文の間に半角スペース＋半角ダッシュ（` — `）等の区切りを入れ、自動リンク化の対象がURL単体に収まるようにする。

例: `https://code.claude.com/docs/en/hooks.md（PreCompactイベントの仕様）`（誤） → `https://code.claude.com/docs/en/hooks.md — PreCompactイベントの仕様`（正）

代替策として、コードスパンで囲む方法（項目18・19と同じ）も有効と推測されるが未検証。

### 教訓

反映後は`notion-fetch`で再取得し、URLの直後に説明文を続ける箇所ではリンク範囲が意図通りか確認する。
